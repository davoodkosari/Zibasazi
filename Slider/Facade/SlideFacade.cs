...
namespace Radyn.Slider.Facade
{
    internal sealed class SlideFacade : SliderBaseFacade<Slide>, ISlideFacade
    {
        ...
        public async Task<Slide> GetSlideWithSlidersAsync(short slideId)
        {
            try
            {
                Slide slide =await new SlideBO().GetAsync(ConnectionHandler, slideId);
                if (slide == null || !slide.Enabled)
                {
                    return null;
                }

                slide.SlideItems =await new SlideItemBO().OrderByAsync(ConnectionHandler, x => x.Order, x =>
