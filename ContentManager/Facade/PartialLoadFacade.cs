...
namespace Radyn.ContentManager.Facade
{
    internal sealed class PartialLoadFacade : ContentManagerBaseFacade<PartialLoad>, IPartialLoadFacade
    {
        ...
        public async Task<List<PartialLoad>> GetHtmlPartialsAsync(Guid htmlId, string culture, Container DefaultContrainer = null)
        {
            try
            {

                var list =await new PartialLoadBO().WhereAsync(this.ConnectionHandler, load => load.HtmlDesginId == htmlId);
                var partialModels = new List<PartialLoad>();
                var contentBo = new ContentBO();
                var contentContentBo = new ContentContentBO();
                var partialsBo = new PartialsBO();
                ...
                if (contenetidlist.Any())
                {
                    contents = await contentBo.WhereAsync(ConnectionHandler, x => x.Id.In(contenetidlist));
                    ContentContents =await contentContentBo.WhereAsync(ConnectionHandler, x => x.Id.In(contenetidlist) && x.LanguageId == culture);
                }

                if (partialidlist.Any())
                    partialses =await partialsBo.WhereAsync(ConnectionHandler, x => x.Id.In(partialidlist));
                foreach (var partialLoad in list)
                {
                    var partialModel = new PartialLoad();
                    if (partialLoad.PartialId.ToInt() > 0)
        ...
        public bool SwapPartials(string partialId, string customId, Guid htmlId, string type)
        {
            try
            {
                this.ConnectionHandler.StartTransaction(IsolationLevel.ReadUncommitted);
                var partialLoadBo = new PartialLoadBO();
                partialLoadBo.SwapPartials(this.ConnectionHandler, partialId, customId, htmlId, type);
                this.ConnectionHandler.CommitTransaction();
                return true;
            }
            catch (KnownException ex)
