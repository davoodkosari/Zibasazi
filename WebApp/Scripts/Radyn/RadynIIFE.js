...
function Close_thisModal() {
    var validId = GetFormId(true);
    var parent = $("#" + "Div" + validId).parent();
    if (parent != undefined) {
        parent.find("#" + "Div" + validId).remove();
        var backdrop = $(".modal-backdrop")[0];
    ...
    var countform = GetFormCount(true);
    if (countform == 0) {
        $("body").removeClass("modal-open");
        $("body").removeClass("main-modal-open");
    }
}
function GetFormId(onlymodal) {
    var index = 1;
    var validId = $("#RadynFormId").val();
    var countform = $("input[Id='RadynFormId']").length;
    $("[Id='RadynFormId']").each(function () {
        var ismodal = $(this).attr("ModalMode");
        if (((ismodal == "True" && onlymodal == true) || (onlymodal == false && ismodal == "False") || onlymodal == undefined) && index == countform) {
            validId = $(this).val();
            return;
        }
        index++;
...
function GetFormCount(onlymodal) {
    var index = 0;
    $("[Id='RadynFormId']").each(function () {
        var ismodal = $(this).attr("ModalMode");
        if (((ismodal == "True" && onlymodal == true) || (onlymodal == false && ismodal == "False") || onlymodal == undefined)) {
            index++;
        }
...
function RadynPostThisForm(posturl, afterfunction) {
    ShowPostBackLoader();
    var validId = GetFormId();
    $.post(posturl, $('form[id="' + validId + '"]').serialize(), afterfunction).always(function () {
        ShowRadynMessage();
        HidePostBackLoader();
    });
}
...
function ShowModalWithReturnValue(url, data, hidId, txtDesc) {
    url = FixUrl(url, data, true);
    var querystring = "";
...
function ShowModalWithReturnValueWithEvents(url, data, hidId, txtDesc, evenetclick) {
    url = FixUrl(url, data, true);
    var querystring = "";
    if (url.indexOf("?") > 0) querystring += "&";
    else querystring += "?";
...
function ShowModalInOtherTagWithReturnValue(url, data, tag, hidId, txtDesc) {
    url = FixUrl(url, data, true);
    var querystring = "";
    if (url.indexOf("?") > 0) querystring += "&";
    else querystring += "?";
...
function ShowModalInOtherTagWithReturnValueWithEvents(url, data, tag, hidId, txtDesc, evenetclick) {
    url = FixUrl(url, data, true);
    var querystring = "";
    if (url.indexOf("?") > 0) querystring += "&";
    else querystring += "?";
