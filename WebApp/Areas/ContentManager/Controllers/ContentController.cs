...
{
    public class ContentController : LocalizedController
    {
        [RadynAuthorize]
        ...
        {
            var content =await ContentManagerComponent.Instance.ContentFacade.GetAsync(id);
            if (content == null) return View();
            ViewBag.Title = content.GetContent(SessionParameters.Culture).Title;
            ViewBag.Html =await ContentManagerComponent.Instance.ContentFacade.GetHtmlAsync(id, SessionParameters.Culture);
            ViewBag.Script = content.UserScript;
            return View();
        }
        ...
        public async Task<ActionResult> ViewPartialAsync(int id)
        {
            ViewBag.Html =await ContentManagerComponent.Instance.ContentFacade.GetHtmlAsync(id, SessionParameters.Culture);
            return PartialView("ViewPartial");
        }
