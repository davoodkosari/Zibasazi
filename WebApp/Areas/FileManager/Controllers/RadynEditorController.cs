...
namespace Radyn.WebApp.Areas.FileManager.Controllers
{
    ...
    public class RadynEditorController : LocalizedController
    {
        ...
        public ActionResult InsertLink(string editorId, Guid folderId)
        {
            try
            {
                ...
                {
                    if (Request.Url == null ||
                           string.IsNullOrEmpty(Request.Url.Authority))
                        throw new Exception(Resources.Common.ErrorInInsert);
                    var id = StatisticComponents.Instance.WebSiteFacade.GetByUrlForEditor(System.Web.HttpContext.Current.Request.Url.Authority);
                    if (id == Guid.Empty)
                    {
        ...
        public ActionResult EditorFolder(bool returnimage = false)
        {
            try
            {
                ...
                if (Request.Url == null ||
                       string.IsNullOrEmpty(Request.Url.Authority))
                    throw new Exception(Resources.Common.ErrorInInsert);
                var id = StatisticComponents.Instance.WebSiteFacade.GetByUrlForEditor(Request.Url.Authority);
                if (id == Guid.Empty)
                {
                    ViewBag.ErrorMessage = "لطفا وب سایت مورد نظر را ثبت نمایید";
        ...
        public ActionResult EditorFolderTree(Guid folderId)
        {
            ...
            var folders = SessionParameters.CurrentCongress!=null?CongressComponent.Instance.BaseInfoComponents.CongressFoldersFacade.Select(x => x.Folder,
                x => x.CongressId == SessionParameters.CurrentCongress.Id,
                new OrderByModel<CongressFolders>() { Expression = x => x.Folder.Title }, true):new List<Folder>();
            foreach (var folder in folders)
            {
                if(list.All(x => x.Id != folder.Id))
                    list.Add(folder);
            }
