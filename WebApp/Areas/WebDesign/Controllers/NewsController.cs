...
namespace Radyn.WebApp.Areas.WebDesign.Controllers
{
    public class NewsController : WebDesignBaseController
    {
        ...
        {
            var list = WebDesignComponent.Instance.NewsFacade.Select(x => x.WebSiteNews, x => x.WebId == this.WebSite.Id);
            return View(list);
        }
        [RadynAuthorize]
        ...
        public ActionResult Delete(Int32 id, FormCollection collection)
        {
            try
            {
                if (WebDesignComponent.Instance.NewsFacade.Delete(this.WebSite.Id, id))
                {
