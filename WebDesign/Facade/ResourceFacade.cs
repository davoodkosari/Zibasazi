...
namespace Radyn.WebDesign.Facade
{
    internal sealed class ResourceFacade : WebDesignBaseFacade<Resource>, IResourceFacade
    {
        ...
        public string GetWebSiteResourceHtml(Guid websiteId, string culture)
        {
            try
            {
                ...
                var byFilter = new ResourceBO().OrderBy(this.ConnectionHandler, x => x.Order,
                    x => x.WebId == websiteId && x.LanuageId == culture);
